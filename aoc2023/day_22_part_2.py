...
def solution(input_rel_uri):
    ...
    return sum(
        disintegration_score(
            {brick},
            {brick},
            supported_bricks_per_brick,
            supporting_bricks_per_brick,
        )
        for brick in bricks
    )
...
def disintegration_score(
    latest_collapsed_bricks,
    all_collapsed_bricks,
    supported_bricks_per_brick,
    supporting_bricks_per_brick,
):
    newly_collapsed_bricks = {
        supported_brick
        for latest_collapsed_brick in latest_collapsed_bricks
        for supported_brick in supported_bricks_per_brick[latest_collapsed_brick]
        if supporting_bricks_per_brick[supported_brick].issubset(all_collapsed_bricks)
    }
    if not newly_collapsed_bricks:
        return 0
    return disintegration_score(
        newly_collapsed_bricks,
        all_collapsed_bricks.union(newly_collapsed_bricks),
        supported_bricks_per_brick,
        supporting_bricks_per_brick,
    ) + len(newly_collapsed_bricks)
