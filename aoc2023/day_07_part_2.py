...
def solution(input_rel_uri):
    ...
    hand_to_ordering = {
        hand: strongest_ordering_with_morphing_jokers(hand) for hand in hand_to_bid
    }
    return sum(
...
def strongest_ordering_with_morphing_jokers(hand):
    def non_joker_cards_reverse_sorted_distribution(hand):
        return sorted(
            Counter(card for card in hand if card != "J").values(), reverse=True
        )
    ...
    match hand.count("J"):
        case 5 | 4:
            ret = ordering_of_type(HandType.FIVE_OF_A_KIND)
        case 3:
            match non_joker_cards_reverse_sorted_distribution(hand):
                case [2]:
                    ret = ordering_of_type(HandType.FIVE_OF_A_KIND)
                case [1, 1]:
                    ret = ordering_of_type(HandType.FOUR_OF_A_KIND)
